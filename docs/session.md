...
Сервер узнал нашего пользователя по присланным cookie, и дальше предоставит ему доступ к личной информации.
...
Зайдите в браузере на страницу http://127.0.0.1:8080/session.start.php, вы должны увидеть лишь пустую страницу, но не спешите
...
Следующий шаг - сохраним в сессию произвольное значение, для этого в PHP используется супер-глобальная
...
Так, что мы ещё не пробовали? Правильно - украсть "печеньки", давайте запустим другой браузер и добавим в него теже самые cookie.
...
> Во второй строке странная "магия" - это фокус с буфером вывода, я ещё расскажу о нём в одной из следующих статей,
  пока считайте это лишь строкой длинной в 4096 символов, в данном случае - это всё пробелы
...
будьте внимательны, и вас сия зараза не коснется (как-же, ... гомерический смех).
...
В этой статье вам дано пять заданий, при этом они касаются не только работы с [сессиями](http://php.net/ref.session),
но так же познакомят вас с [MySQL](http://php.net/book.mysqli) и с [функциями работы со строками](http://php.net/ref.strings).
Для усвоения этого материала - отдельной статьи не нужно, хватит и мануала по приведенным ссылкам - никто за вас его читать
не будет.
